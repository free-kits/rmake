...
export const findFileToNavs = () => {
    ...
    logger.info('find md files.', files);
    const navs: Nav[] = [];
    files.forEach((file) => {
        ...
        if (mdInfo?.length === 1) {
            ...
            const groupInfo = {
                ...
                pages: [{
                    ...
                    path: `/*@freekits/doc import*/ ${file.replace(process.cwd(), '')}`,
                }],
            };
...
export const createRouteConfig = () => {
    ...
    writeFileSync(join(process.cwd(), '.doc', 'config.ts'), `export default () => ${JSON.stringify(navs)}`);
};
